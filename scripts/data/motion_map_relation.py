...
followees_dir = Path(meta_data_path) / 'rel_motion_map' / 'followees'
followers_dir = Path(meta_data_path) / 'rel_motion_map' / 'followers'
main_dir = Path(meta_data_path) / 'rel_motion_map' / 'main'

save_rel_mo_dir.mkdir(exist_ok=True)
followees_dir.mkdir(exist_ok=True)
...
all_rel_tracks = load_and_merge_json([
    osp.join(meta_data_path, "relation", "neighbor_tracks.json"),
])

## All neighbors mapping
all_rel_mapping = load_and_merge_json([
    osp.join(meta_data_path, "relation", "train_tracks_relation.json"),
    osp.join(meta_data_path, "relation", "test_tracks_relation.json"),
    osp.join(meta_data_path, "relation", "pseudo_test_tracks_relation.json")
])
...
def get_rel_motion_map(info):
    ...
    FOLLOWEE_RATE = 1.5 # reduce to get longer tracklet of followeee
