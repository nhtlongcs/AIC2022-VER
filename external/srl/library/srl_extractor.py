...
class SRL(object):
  def __init__(self, path=None, filetype=1):
    self.srl_helper = SRLHelper(osp.join(DATA_DIR, 'srl/srl_helper/srl_helper.json'), get_predictor=True)
    if path is not None:
      data_helper = DataHelper()
      self.data = data_helper.load_file(path)
      if filetype == 1:
        self.file_note = 'train'
  ...
  def get_subject_phrase(self, pos_part):
    subject_phrase = None
    sub_pos_part = self.extract_sub_pos_part(sub_pos_part=pos_part[0], indicator=':', distance=2)
  ...
  def get_action_feature(self, pos_part):
    ...
    for i in range(verb_pos):
      subject = self.extract_sub_pos_part(pos_part[i], content_indicator, distance)
      list_vehicle = self.srl_helper.list_vehicle
    ...
    for i in range(len(pos_part)):
      if i == flag:
        continue
      feature = self.extract_sub_pos_part(pos_part[i], content_indicator, distance)
      semantic_indicator = feature[0]
      for key_indicator in self.srl_helper.semantic_key_converter:
        if self.srl_helper.semantic_key_converter[key_indicator] == semantic_indicator:
  ...
  def compare_subject_object_from_all_queries(self, ans):
    for key_group_query in ans:
      for key_query in ans[key_group_query]:
        ...
        ans_query = self.compare_subject_object_from_single_query(ans_query, ans_query['main_subject'])
  ...
  def compare_subject_object_from_single_query(self, ans_query, subject):
    ...
    for i in range(len(ans_srl)):
      ...
      if any(key in subject_srl_item for key in key_item) and (not any(ans_query["cleaned_caption"].find(skip) == 0 for skip in skip_item_query)) and (not any(subject_srl_item.find(skip) == 0 for skip in skip_item)):
